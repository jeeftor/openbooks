...
- **Surface all IRC channel join errors to the user.** Previously, 471 (channel full), 473 (invite-only), 475 (bad channel key), and 433 (nick in use) were logged to stdout but the UI hung silently. Now all four are detected as events and surfaced as error toasts with clear messages explaining what went wrong.
